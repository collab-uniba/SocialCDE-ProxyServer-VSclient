    ...
    public partial class UIRegistration : UserControl
    {
        ...
        private void RegisterButton_Click(object sender, RoutedEventArgs e)
        {
            if (UIController.Proxy == null)
            {
                ...
                ErrorLabel.Content = "Please insert a valid host for the proxy";
            }
            else if (!CheckPassword())
            {
                ErrorBorder.Background = Brushes.Red;
                ErrorRow.Height = GridLength.Auto;
                ErrorLabel.Content = "Please insert a valid password";
            }
            else
            {
                _email = Email.Text;
        ...
        private void Register()
        {
            ...
            switch (res)
            {
                ...
                case 1: // if e-mail address does not exist in the database
                    error = "Please insert the email on which you recived the invite";
                    break;
                case 2: // if password does not match with the e-mail address sent
                    error = "Please insert the invitation code that you recived in the invite";
                    break;
                case 3: // if username is already used by another user
                    error = "The Username chosen is not aviable";
                    break;
            }
            this.Dispatcher.BeginInvoke(new Action(delegate()
            {
        ...
        private void ProxyHost_LostFocus(object sender, RoutedEventArgs e)
        {
            if (String.IsNullOrEmpty(ProxyHost.Text))
            {
                ...
                if (!String.IsNullOrEmpty(UserName.Text))
                {
                    ...
                    UsernameErrorLabel.Content = "Insert a correct Proxy Host to check if the Username is aviable";
                }
            }
            else
            {
        ...
        /// <summary>
        /// Check if the Proxy Host inserted is correct.
        /// </summary>
            ...
            this.Dispatcher.BeginInvoke(new Action(delegate()
            {
                ...
                if (UIController.Proxy == null)
                {
                    if (!UserName.Text.Equals(String.Empty))
                    {
                        UsernameErrorBorder.Background = Brushes.Red;
                        UsernameErrorRow.Height = GridLength.Auto;
                        UsernameErrorLabel.Content = "Insert a correct Proxy Host to check if the Username is available";
                    }
                }
                else
                {
        ...
        /// <summary>
        /// Check if the Username is aviable.
        /// </summary>
        private void CheckUsername()
        {
            ...
            {
                error = "Username aviable";
                backgroundColor = Brushes.Green;
            }
            else
            {
                error = "The Username is not aviable";
