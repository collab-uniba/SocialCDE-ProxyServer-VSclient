...
namespace It.Uniba.Di.Cdg.SocialTfs.Client
{
    ...
    internal class SocialTFSProxyClient : ClientBase<ISocialTFSProxy>, ISocialTFSProxy
    {
        ...
        WEdu[] ISocialTFSProxy.GetEducations(string username, string password, string ownerName)
        {
            try
            {
                if (UIController.Connected)
                    return Channel.GetEducations(username, password, ownerName);
                return new WEdu[0];
            }
            catch
            {
        ...
        WPos[] ISocialTFSProxy.GetPositions(string username, string password, string ownerName)
        {
            try
            {
                if (UIController.Connected)
                    return Channel.GetPositions(username, password, ownerName);
                return new WPos[0];
            }
            catch
            {
        ...
        WReputation ISocialTFSProxy.GetReputations(string username, string password, string ownerName)
        {
            try
            {
                if (UIController.Connected)
                    return Channel.GetReputations(username, password, ownerName);
