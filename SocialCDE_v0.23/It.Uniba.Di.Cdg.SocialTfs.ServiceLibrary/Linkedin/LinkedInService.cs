...
namespace It.Uniba.Di.Cdg.SocialTfs.ServiceLibrary.Linkedin
{
    ...
    class LinkedInService : OAuth2Service, IService
    {
        ...
        List<FeaturesType> IService.GetPublicFeatures()
        {
            ...
            features.Add(FeaturesType.Educations);
            features.Add(FeaturesType.Positions);
        ...
        object IService.Get(FeaturesType feature, params object[] param)
        {
            ...
            switch (feature)
            {
                ...
                case FeaturesType.Skills:
                    result = GetSkills();
                    break;
                case FeaturesType.Educations:
                    result = GetEducations();
                    break;
                case FeaturesType.Positions:
                    result = GetPositions();
                    break;
        ...
        string GetOAuthData(string Service_name, string host, string consumerKey, string consumerSecret, string accessToken)
        {
            ...
            {
                return "https://www.linkedin.com/uas/oauth2/authorization?response_type=code&client_id=" + consumerKey + "&scope=r_fullprofile%20r_network&state=collab2013&redirect_uri=https://www.linkedin.com/";

            }
            else
            {
                String jsonResponse = WebRequestPost("https://www.linkedin.com/uas/oauth2/accessToken?grant_type=authorization_code&code=" + accessToken + "&redirect_uri=https://www.linkedin.com/&client_id=" + consumerKey + "&client_secret=" + consumerSecret, new Dictionary<string, string>());
        ...
        IPos[] GetPositions()
        {
            String jsonPositions = WebRequestLinkedIn(_host + LinkedInUri.Default.POSITIONS);
            LinkedInPositions positions = (jsonPositions != null || jsonPositions != "" ? JsonConvert.DeserializeObject<LinkedInPositions>(jsonPositions) : null);
            if (positions == null)
            {
                return null;
            }
        ...
        IEdu[] GetEducations()
        {
            String jsonEducations = WebRequestLinkedIn(_host + LinkedInUri.Default.EDUCATIONS);
            if ( String.IsNullOrEmpty ( jsonEducations ) ) return null;
            LinkedInEducations educations = JsonConvert.DeserializeObject<LinkedInEducations>(jsonEducations);
            return educations.educations.values;
        }
