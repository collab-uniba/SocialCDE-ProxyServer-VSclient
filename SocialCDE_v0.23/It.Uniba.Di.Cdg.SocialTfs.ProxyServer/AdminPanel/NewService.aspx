    ...
    <script type="text/javascript">
        ...
        function checkHost() {
            if ($("#MainContent_HostRW").is(":visible")) {
                if ($("#MainContent_HostTB").val() == "") {
                    ...
                    var regexp = /^(http(s)?:\/\/)?([\w-]+\.{1})*(([\w-]*){1}(\.{1}[A-Za-z]{2,4}){1}){1}(\:{1}\d*)?([\?\/|\#]+[\@\~\;\+\'\#\,\.\%\-\/\&\?\=\w\$\s]*)*$/i;
                    if (regexp.test($("#MainContent_HostTB").val())) {
                        invalidHost = false;
                        $("#MainContent_ErrHostRW").hide();
                    }
                    else {
                        invalidHost = true;
                        $("#MainContent_ErrHostRW").show();
                        $("#HostER").html("Please, use a valid public url.");
                    }
                }
            }
        }
        ...
        $(document).ready(function () {
            ...
            $("#MainContent_Submit").click(function () {
                ...
                if (!emptyName && !emptyHost && !invalidHost && !emptyKey && !emptySecret)
                    $("#ctl01").submit();
                else
